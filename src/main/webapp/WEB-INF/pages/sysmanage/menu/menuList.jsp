	...
	$(function() {
 	...
 	var menuMgr = { 	    		
		delMenu:function(menuId){
			top.layer.confirm("您确定要删除此菜单吗?", {icon: 3 ,title: "系统提示"} ,function(index){
 		 		  $.ajax({
 						type:'post',//请求方式
 						...
 						success:function(data){//与服务器交互成功调用的回调函数
 							...
 							//alert(data.result);
 							top.layer.alert(data.result, {icon: 0, title:'提示'});
...
<body>
	...
	<form id="menuListForm" method="post" action="${ctx}/sysmgr/menu/gotoMenuList">
				...
				<c:forEach items="${menuList}" var="menu">
					<tr id="${menu.id}" pId="${menu.parentId}">
						<td nowrap>
							...
							<a href="${ctx}/sysmgr/menu/gotoMenuEdit?editFlag=2&&menuId=${menu.id}">${menu.name}</a></td>

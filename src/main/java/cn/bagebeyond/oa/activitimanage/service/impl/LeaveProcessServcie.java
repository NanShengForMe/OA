...
import cn.bagebeyond.oa.activitimanage.mapper.LeaveBeanMapper;
...
@Service
public class LeaveProcessServcie implements ILeaveProcessService {
	...
	public List<LeaveBean> getLeaveBeanList(LeaveBean leaveBean) {
		return this.leaveBeanMapper.getLeaveBeanList(leaveBean);
	}
	...
	public boolean addLeaveBean(LeaveBean leaveBean) {
		...
		return this.leaveBeanMapper.addLeaveBean(leaveBean);
	}
	 ...
	 * 请假单录入以后,根据流程图,启动流程
	 * 启动流程需要做以下几件事情
	 *	1:获取请假记录,更改请假单状态(流程一旦启动,请假单信息不能再次编辑)	 
	...
	public void doLeaveProcess(Long leaveId) {
		...
		// 构造businessKey --将本次的请假单对象与流程实例产生对应关系
		...
		this.leaveBeanMapper.updateLeaveBeanInstanceId(leaveId, processInstanceId);
	}
	...
	public LeaveBean getLeaveBeanByTaskId(String taskId) {
		...
		LeaveBean leaveBean = this.leaveBeanMapper.getLeaveBeanById(new Long(leaveId));
		return leaveBean;
	}
